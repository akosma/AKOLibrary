...
* Run the tests.
* Add a new header file for the group (if required), similar to the files
"AKOLibrary_Video.h".
...
* When the compilation is ready, open the "Products" group and right-click
on the AKOLibrary.framework product. Select "Show in Finder".
* In the window that appears, go back a few levels in the folder hierarchy
...
* Import the AKOLibrary.framework bundle in your app. Choose to explicitly
copy it into the project.
* Select the project icon, select the main target and select the "Build Phases"
